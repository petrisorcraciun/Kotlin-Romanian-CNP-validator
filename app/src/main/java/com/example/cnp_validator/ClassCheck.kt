...
class ClassCheck {
    ...
    {
        var sum:Int = 0
        for (i in 0..testKey.length-1)
        {
            sum += CNP.substring(i, i+1).toInt() * testKey.substring(i, i+1).toInt()
        }

        val rest:Int = sum%11

        if(rest<10 && rest == CNP.substring(12, 13).toInt())
        {
            return true
        }else if(rest == 10 && CNP.substring(12, 13).toInt() == 1){
            return true
        }
        return false
    }
